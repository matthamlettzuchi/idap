// app/credit-simulation/page.tsx
"use client";
...
// Batas Maksimal Plafon: 100 Triliun (100,000,000,000,000)
const MAX_AMOUNT = 100000000000000;

export default function CreditSimulationPage() {
  const [amount, setAmount] = useState<number>(100000000); // Default 100 Juta
  ...
  // Helper pembersih string input & enforcer batas MAX_AMOUNT
  const parseAndClampNumber = (val: string): number => {
    ...
    // Terapkan Limit Maksimal 100 Triliun
          ...
          <div className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-12">
            ...
            <div className="space-y-8 rounded-2xl border border-[var(--panel-border)] bg-panel p-6 md:p-8 lg:col-span-7">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-[13.5px] font-semibold text-ink-1 flex items-center gap-2">
                    <DollarSign size={16} className="text-signal-teal" /> Plafon Pinjaman (Amount)
                  </label>
                  <span className="font-mono text-[11px] text-ink-2">Maks. Rp 100 Triliun</span>
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-full rounded-xl border border-[var(--panel-border)] bg-panel-2 py-3.5 pl-12 pr-4 font-mono text-xl font-bold text-ink-0 transition-all focus:border-signal-teal focus:outline-none focus:ring-1 focus:ring-signal-teal"
                  />
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-[13.5px] font-semibold text-ink-1 flex items-center gap-2">
                    <Calendar size={16} className="text-signal-teal" /> Period / Tenor (Bulan)
                  </label>
                  <div className="flex items-center gap-1.5">
                    ...
                    />
                    <span className="text-[12px] text-ink-2 font-medium">Bulan</span>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-[13.5px] font-semibold text-ink-1 flex items-center gap-2">
                    <Percent size={16} className="text-signal-teal" /> Suku Bunga (% pertahun)
                  </label>
                  <div className="flex items-center gap-1.5">
                    <input
                      type="number"
            ...
            <div className="flex flex-col justify-between overflow-hidden rounded-2xl border border-[var(--panel-border)] bg-panel p-6 md:p-8 lg:col-span-5">
              <div>
                <div className="flex items-center justify-between border-b border-[var(--panel-border)] pb-4">
                  <span className="mono-label">Hasil Kalkulasi</span>
                ...
                <div className="mt-6 rounded-xl border border-[var(--panel-border)] bg-panel-2 p-5 text-center overflow-hidden">
                  <div className="text-[13px] font-medium text-ink-2">Estimasi Rental Bulanan</div>
                  ...
                  >
                    <span className="text-base font-normal text-ink-2 shrink-0">Rp</span>
                    {/* Menggunakan break-all dan fluid font agar teks sepanjang apa pun tetap terurai di dalam box */}
                  ...
                  <div className="mt-2 text-[12px] text-ink-2">
                    per bulan selama <span className="font-semibold text-ink-0">{period} bulan</span>
                  </div>
                </div>
                ...
                <div className="mt-6 space-y-4 border-t border-[var(--panel-border)] pt-5 text-[13px]">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                    <span className="text-ink-2 shrink-0">Pokok Pinjaman:</span>
                    <span className="font-mono font-medium text-ink-0 break-all text-right">
                      Rp {formatNumber(amount)}
                    </span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                    <span className="text-ink-2 shrink-0">Estimasi Total Bunga:</span>
                    <span className="font-mono font-medium text-ink-0 break-all text-right">
                      Rp {formatNumber(totalInterest)}
                    </span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 pt-2 border-t border-[var(--panel-border)]">
                    <span className="font-semibold text-ink-1 shrink-0">Total Pengembalian:</span>
              ...
              <div className="mt-8 pt-5 border-t border-[var(--panel-border)] space-y-3">
                <Button asChild variant="primary" className="w-full flex justify-center items-center gap-2 text-[14px]">
                  <a href="/contact">
                    Konsultasi Integrasi Core System <ArrowRight size={16} />
                  </a>
                </Button>
                <p className="text-center text-[11px] text-ink-2 leading-relaxed">
                  *Perhitungan di atas merupakan simulasi estimasi awal. Nilai riil dapat disesuaikan berdasarkan regulasi dan skema suku bunga perusahaan pembiayaan Anda.
                </p>
              </div>
            </div>
          </div>
